...
const Content = ({
  ...
  return (
    ...
    >
      <div
        className={`absolute z-50 mt-2 rounded-md shadow-lg border border-[rgb(var(--border))] ${alignmentClasses} ${widthClasses}`}
...
const DropdownLink = ({ className = '', children, ...props }: InertiaLinkProps) => {
  return (
    <Link
      {...props}
      className={
        'block w-full px-4 py-2 text-start text-sm leading-5 text-app transition duration-150 ease-in-out hover:bg-surface-2 focus:bg-surface-2 focus:outline-none ' +
        className
      }
    >
      {children}
